...
$gif = $_FILES['gif']['tmp_name'];
$extractor = new GifFrameExtractor();
if(!GifFrameExtractor::isAnimatedGif($gif)){
	echo "not animated gif";

	return;
}
	

$extractor->extract($gif, true);
...
$output_data = array();
$output_data['frames'] = array();

$i = 0;
$first_frame;
$pos = $extractor->getFramePositions();

foreach ($extractor->getFrames() as $frame) {
	if($i == 0){
		$output_data['frame_rate'] = 100/$frame['duration'];
        $output_data['width'] = imagesx($frame['image']);
        $output_data['height'] = imagesy($frame['image']);
		$first_frame = $frame;
	}
	...
	$output = $first_frame['image'];
	if($i>0){
		imagecopy($output, $frame['image'], $pos[$i]['x'], $pos[$i]['y'], 0, 0, imagesx($frame['image']), imagesy($frame['image']));
	}
	...
	$output_data['frames'][] = 'data: '.'image/gif'.';base64,'.base64_encode($data);
